...
import ayd2.p2b.conference_service_api.feature.call.infrastructure.persistence.adapter.JpaCallCongressAdapter;
...
class JpaCallCongressAdapterTest {
    ...
    private JpaCallCongressAdapter adapter;

    @BeforeEach
    void setUp() {
        adapter = new JpaCallCongressAdapter(entityManager);
    }
    ...
    void shouldReturnManageableCongressSummaryWhenFound() {
        ...
        Optional<CallCongressSummary> result = adapter.findManageableCongressById(congressId);

        assertThat(result).isPresent();
        CallCongressSummary summary = result.orElseThrow();
    ...
    @Test
    void shouldReturnTrueWhenPublicCongressExists() {
        UUID congressId = UUID.randomUUID();
